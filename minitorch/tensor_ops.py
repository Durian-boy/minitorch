...
# 现在有一个张量A，并且我们调用shape_broadcast计算了它和另一个张量B广播后的形状（out）
# 现在要对张量A的元素做运算（fn），并将结果映射为广播后的形状（out）
def tensor_map(
...
# 这里的fn可以看作是add和mul，第三个形参在fn为add时取0.0，在fn取mul时取1.0
def tensor_reduce(
    fn: Callable[[float, float], float]
