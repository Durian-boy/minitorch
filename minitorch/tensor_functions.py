...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (a,) = ctx.saved_values
        # sigmoid(t1)的导数 = sigmoid(t1) * (1 - sigmoid(t1))
        return a.f.mul_zip(grad_output, a.f.mul_zip(a, a.f.add_zip(tensor([1.0]), a.f.neg_map(a))))


class ReLU(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        ctx.save_for_backward(t1)  # 由于要用t1的元素判断是否>=0,所以要存储t1
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (a,) = ctx.saved_values
        return a.f.exp_map(a) * grad_output
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # 对order从小到大进行排序，确保在反向传播时，梯度在传播过程中的顺序与前向传播时的顺序保持一致
        # 因为在没有应用permute之前，order是[0,1,2,3,...]
        new_order: List[int] = [
            a[0] for a in sorted(
...
class MatMul(Function):
    ...
    # 矩阵相乘的求导就是先取转置，再乘以grad，具体公式看minitorch官网
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
