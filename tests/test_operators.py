...
# run pytest tests/test_operators.py -m task0_1 to varify
...
# run pytest tests/test_operators.py -m task0_2 to varify
...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0 <= sigmoid(-a) <= 1.0
    assert eq(1 - sigmoid(a), sigmoid(-a)) == 1.0
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert eq(mul(c, add(a, b)), add(mul(c, a), mul(c, b))) == 1.0
...
def test_other() -> None:
    ...
    """
    assert eq(relu(-1), 0.0) == 1.0
    assert eq(relu(1), 1.0) == 1.0


# ## Task 0.3  - Higher-order functions
# run pytest tests/test_operators.py -m task0_3 to varify
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert eq(add(sum(ls1), sum(ls2)), sum(addLists(ls1, ls2))) == 1.0
